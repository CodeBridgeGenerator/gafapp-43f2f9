...
import proposalRefFakerFactory from "./proposalRefFakerFactory";
...
const ProposalRefFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = proposalRefFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("proposalRef").create(elem));
...
export default connect(mapState, mapDispatch)(ProposalRefFakerDialogComponent);
