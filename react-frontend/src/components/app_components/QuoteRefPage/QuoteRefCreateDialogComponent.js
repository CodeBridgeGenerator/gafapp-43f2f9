...
const QuoteRefCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        const result = await client.service("quoteRef").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info QuoteRef created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in QuoteRef" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create QuoteRef" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="quoteRef-create-dialog-component">
...
export default connect(mapState, mapDispatch)(QuoteRefCreateDialogComponent);
