...
import quoteRefFakerFactory from "./quoteRefFakerFactory";
...
const QuoteRefFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = quoteRefFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("quoteRef").create(elem));
...
export default connect(mapState, mapDispatch)(QuoteRefFakerDialogComponent);
