...
import quotesFakerFactory from "./quotesFakerFactory";
...
const QuotesFakerDialogComponent = (props) => {
...
client.service("users")
    ...
    const onRun = async () => {
        let fakeData = quotesFakerFactory(props.user,fakerCount,projectNameIds,approvedByIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("quotes").create(elem));
...
export default connect(mapState, mapDispatch)(QuotesFakerDialogComponent);
