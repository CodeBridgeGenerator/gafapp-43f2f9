...
const ProjectProposalsPage = (props) => {
  ...
  useEffect(() => {
    ...
    client
      ...
      .find({
        query: {
          $limit: 10000,
          proposalRef: urlParams.singleProposalsId,
          quotation: urlParams.singleQuotesId,
          approvedBy: urlParams.singleUsersId,
          $populate: [
            {
              path: "createdBy",
              service: "users",
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("projectProposals").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
        ...
        <div className="col-6 flex justify-content-end">
          <>
            ...
            <Button
              ...
              role="projectProposals-add-button"
            />
          </>
        </div>
      </div>
