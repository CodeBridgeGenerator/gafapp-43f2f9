...
import ProjectProposalsDatatable from "./ProjectProposalsDataTable";
import ProjectProposalsEditDialogComponent from "./ProjectProposalsEditDialogComponent";
import ProjectProposalsCreateDialogComponent from "./ProjectProposalsCreateDialogComponent";
import ProjectProposalsFakerDialogComponent from "./ProjectProposalsFakerDialogComponent";
import ProjectProposalsSeederDialogComponent from "./ProjectProposalsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const ProjectProposalsPage = (props) => {
    ...
    const filename = "projectProposals";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("projectProposals");
    ...
    useEffect(() => {
        ...
        props.show();
        client
            .service("projectProposals")
            .find({ query: { $limit: 10000 , proposalRef : urlParams.singleProposalsId ,quotation : urlParams.singleQuotesId ,approvedBy : urlParams.singleUsersId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "ProjectProposals", type: "error", message: error.message || "Failed get ProjectProposals" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("projectProposals").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "ProjectProposals", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("projectProposals").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "ProjectProposals",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/projectProposals/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="projectProposals-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="projectProposals-datatable">
                    <ProjectProposalsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ProjectProposalsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ProjectProposalsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ProjectProposalsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ProjectProposalsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ProjectProposalsPage);
