...
import projectProposalsFakerFactory from "./projectProposalsFakerFactory";
...
const ProjectProposalsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [proposalRefIds, setproposalRefIds] = useState();
...
const [approvedByIds, setapprovedByIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("proposals")
                        .find({ query: { $select: ["_id","proposalRef"] } })
                        .then((data) => {
                        setproposalRefIds(data.data);
                        });
client.service("quotes")
                        .find({ query: { $select: ["_id","quoteNo"] } })
                        .then((data) => {
                        setquotationIds(data.data);
                        });
client.service("users")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setapprovedByIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = projectProposalsFakerFactory(props.user,fakerCount,proposalRefIds,quotationIds,approvedByIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("projectProposals").create(elem));
...
export default connect(mapState, mapDispatch)(ProjectProposalsFakerDialogComponent);
