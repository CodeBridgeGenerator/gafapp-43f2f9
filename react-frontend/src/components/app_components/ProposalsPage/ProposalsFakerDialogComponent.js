...
import proposalsFakerFactory from "./proposalsFakerFactory";
...
const ProposalsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [projectNameIds, setprojectNameIds] = useState();
...
client.service("users")
    ...
    const onRun = async () => {
        let fakeData = proposalsFakerFactory(props.user,fakerCount,projectNameIds,approvedByIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("proposals").create(elem));
...
export default connect(mapState, mapDispatch)(ProposalsFakerDialogComponent);
