...
import ProposalsDatatable from "./ProposalsDataTable";
import ProposalsEditDialogComponent from "./ProposalsEditDialogComponent";
import ProposalsCreateDialogComponent from "./ProposalsCreateDialogComponent";
import ProposalsFakerDialogComponent from "./ProposalsFakerDialogComponent";
import ProposalsSeederDialogComponent from "./ProposalsSeederDialogComponent";
...
const ProposalsPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("proposals");
    ...
    useEffect(() => {
        ...
        client
            .service("proposals")
            .find({ query: { $limit: 10000 , projectName : urlParams.singleProjectsId ,approvedBy : urlParams.singleUsersId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("proposals").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Proposals", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("proposals").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="proposals-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="proposals-datatable">
                    <ProposalsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ProposalsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ProposalsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ProposalsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ProposalsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ProposalsPage);
