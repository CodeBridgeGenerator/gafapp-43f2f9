...
const [contact, setContact] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [company,contact], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.relevantAITechnologies)) {
                error["relevantAITechnologies"] = `Relevant AI Technologies field is required`;
                ret = false;
            }
  
            if (_.isEmpty(_entity?.expectedOutcomes)) {
                error["expectedOutcomes"] = `Expected Outcomes / Success Me field is required`;
                ret = false;
            }
            ...
            if (_.isEmpty(_entity?.dataAvailability)) {
                error["dataAvailability"] = `Data Availability & Readiness field is required`;
                ret = false;
            }
  
            if (_.isEmpty(_entity?.requirements)) {
                error["requirements"] = `Integration Deploy Requirement field is required`;
                ret = false;
            }
            ...
            if (_.isEmpty(_entity?.referredCollaborationTimeline)) {
                error["referredCollaborationTimeline"] = `Referred Collaboration Timeline field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            company: _entity?.company?._id,contact: _entity?.contact?._id,projectName: _entity?.projectName,idGAF: _entity?.idGAF,title: _entity?.title,problemStatement: _entity?.problemStatement,relevantAITechnologies: _entity?.relevantAITechnologies,expectedOutcomes: _entity?.expectedOutcomes,currentSolutions: _entity?.currentSolutions,targetUsers: _entity?.targetUsers,dataAvailability: _entity?.dataAvailability,requirements: _entity?.requirements,budgetAllocation: _entity?.budgetAllocation,referredCollaborationTimeline: _entity?.referredCollaborationTimeline,tags: _entity?.tags,founded: _entity?.founded,headquarters: _entity?.headquarters,deadline: _entity?.deadline,daysRemaining: _entity?.daysRemaining,budget: _entity?.budget,category: _entity?.category,enterprise: _entity?.enterprise,posted: _entity?.posted,others: _entity?.others,path: _entity?.path,
